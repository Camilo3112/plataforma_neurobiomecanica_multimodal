...
\title{\textbf{Metodología de Procesamiento Multimodal para Tomografía, Tractografía, Corteza Motora, EMG y Dinamometría}\\
\large Pipeline VCE: transformaciones, fundamentos matemáticos, atlas, CST y métricas}
...
% ============================================================
\section{Resumen general del procesamiento}
...
\begin{itemize}
    \item TAC: músculo, grasa, áreas, volúmenes y landmarks.
    \item Resonancia / Brain00mm: ubicación anatómica y corteza motora.
    \item Difusión / tractografía: reconstrucción propia de streamlines y selección anatómica de la vía corticoespinal.
    \item Atlas / FreeSurfer / MNI: localización anatómica guiada.
    \item EMG: activación muscular y comportamiento temporal-frecuencial.
    \item Dinamometría: desempeño mecánico.
    \item Estadística: Antes vs. Después, paciente vs. sano y estabilidad del protocolo.
\end{itemize}

% ============================================================
\section{Procesamiento de tomografías}
...
Los cortes se ordenan usando su posición espacial, normalmente el atributo DICOM \texttt{ImagePositionPatient}. Si \(\mathbf{p}_k\) es la posición física del corte \(k\), el orden se obtiene por la proyección:
...
\subsection{Conversión a Unidades Hounsfield}
...
\begin{itemize}
    \item \(PV\) es el valor crudo del píxel.
    \item \(m\) es \texttt{RescaleSlope}.
    \item \(b\) es \texttt{RescaleIntercept}.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Rangos HU usados conceptualmente para segmentación}
\begin{tabular}{ll}
\toprule
...
\subsection{Construcción de la matriz affine}
...
\begin{itemize}
    \item \(\mathbf{r}\) es el vector de orientación de filas.
    \item \(\mathbf{c}\) es el vector de orientación de columnas.
    \item \(\mathbf{n}=\mathbf{r}\times\mathbf{c}\) es el vector normal al corte.
    \item \(\Delta_i,\Delta_j\) son los tamaños de píxel.
    \item \(\Delta_k\) es el espaciamiento entre cortes.
    \item \((x_0,y_0,z_0)\) es la posición física del primer corte.
\end{itemize}
...
% ============================================================
\section{Detección anatómica y landmarks en TAC}

\subsection{Segmentación inicial de hueso}
...
\subsection{Detección de trocánter y punto tibial anteromedial}
...
% ============================================================
\section{Segmentación muscular}

\subsection{Separación por lado}
...
\subsection{Ray casting desde el hueso}
...
\subsection{Transformada wavelet continua para bordes}
...
\begin{itemize}
    \item \(a\) es la escala.
    \item \(b\) es la posición.
    \item \(\psi\) es la wavelet madre.
    \item \(^\ast\) denota conjugado complejo.
\end{itemize}
...
\subsection{Construcción de la máscara muscular}
...
% ============================================================
\section{Segmentación de tejido adiposo}

\subsection{Grasa subcutánea}
...
\subsection{Volumen de grasa subcutánea}
...
\subsection{Grasa intramuscular}
...
\subsection{Relaciones grasa/músculo}
...
% ============================================================
\section{Exportación de máscaras NIfTI y labelmaps}
...
% ============================================================
\section{Procesamiento de resonancia y corrección cortical}

\subsection{Problema de alineación}
...
\subsection{Remuestreo al espacio de referencia}

Sea \(I_B\) la imagen anatómica de referencia, por ejemplo \texttt{Brain00mm.nii}, y sea \(R\) la máscara de corteza motora original. Cada una tiene su propia grilla y affine:
...
\subsection{Detección de máscara cerebral}
...
\subsection{Capa cortical externa}
...
\subsection{Corrección por traslación}
...
\begin{itemize}
    \item \(s\) es el factor de escala.
    \item \(\mathbf{c}\) es el centroide de la ROI.
    \item \(\mathbf{t}\) es la traslación.
\end{itemize}
...
\subsection{Función de puntuación de la corrección}
...
% ============================================================
\section{Atlas cerebral}

\subsection{Definición de atlas}
...
\subsection{Atlas-prior heurístico}
...
\begin{itemize}
    \item Rápido.
    \item No requiere instalación pesada.
    \item Útil como respaldo cuando no existe FreeSurfer.
\end{itemize}

Limitaciones:

\begin{itemize}
    \item No identifica surcos ni giros reales del paciente.
    \item No reemplaza un registro anatómico atlas-paciente.
    \item Requiere validación visual.
\end{itemize}

\subsection{FreeSurfer como atlas anatómico nativo}
...
\subsection{Tipos de transformaciones de registro}

\subsubsection{Transformación rígida}
...
% ============================================================
\section{Integración anatómica y funcional de la corteza motora}
...
% ============================================================
\section{Métricas de corteza motora}
...
\subsection{Asimetría interhemisférica}
...
\subsection{Estabilidad Antes--Después}
...
% ============================================================
\section{Procesamiento de difusión, tractografía y vía corticoespinal}

\subsection{Propósito del módulo de tractografía}

El módulo de tractografía se incorporó para reconstruir y visualizar un corredor corticoespinal propio dentro del espacio anatómico del paciente, usando como referencia principal el volumen estructural \texttt{rT1.nii}. El objetivo es anexar al pipeline VCE una representación de la conectividad motora descendente, especialmente de la vía corticoespinal o CST (\textit{corticospinal tract}), sin depender de las máscaras ni de la tractografía final generada automáticamente por el resonador.

El flujo implementado se organiza en cinco etapas:

\begin{enumerate}
    \item diagnóstico de la serie de difusión y verificación de gradientes;
    \item conversión de DICOM de difusión a NIfTI con archivos \texttt{bval} y \texttt{bvec};
    \item reconstrucción de tractografía de cerebro completo;
    \item adaptación de regiones motoras M1/M2 y tronco encefálico al espacio del \texttt{rT1};
    \item filtrado anatómico de streamlines para obtener CST izquierda, CST derecha, mapas de densidad y volúmenes coloreados para revisión en ITK-SNAP.
\end{enumerate}
...
\subsection{Modelo físico de difusión en resonancia magnética}
...
\begin{itemize}
    \item $S(b,\mathbf{g})$ es la señal medida con ponderación de difusión;
    \item $S_0$ es la señal sin ponderación de difusión o imagen $b=0$;
    \item $b$ es el factor de ponderación de difusión;
    \item $\mathbf{g}$ es la dirección del gradiente de difusión;
    \item $D$ es el tensor de difusión $3\times3$.
\end{itemize}
...
\subsection{Tensor de difusión y direcciones principales}
...
\subsection{Reconstrucción de streamlines}
...
\subsection{Registro de difusión al espacio anatómico T1}

Para que las streamlines puedan visualizarse junto al \texttt{rT1.nii}, se transforman desde el espacio de difusión al espacio anatómico. Si $\mathbf{x}_{DWI}$ es un punto de una streamline en el espacio de difusión, su posición en T1 se expresa como:
...
El resultado final debe conservar la geometría del \texttt{rT1}: misma matriz affine, mismo tamaño de voxel y misma orientación espacial. Esto permite abrir simultáneamente \texttt{rT1.nii}, mapas de densidad, mapas RGB y máscaras de CST en ITK-SNAP sin desalineación espacial.

\subsection{Definición anatómica de la vía corticoespinal}
...
\subsection{Corrección ventral y paramediana en tronco encefálico}
...
\subsection{Mapas de densidad y visualización RGB}

Además del archivo \texttt{.trk}, la tractografía se voxeliza para generar mapas NIfTI de densidad. Si $v$ es un voxel y $\Gamma_C$ un conjunto de streamlines filtradas, la densidad se calcula como:
...
\subsection{Métricas derivadas de tractografía}
...
\subsection{Control de calidad específico de tractografía}

El control de calidad de este módulo incluye:

\begin{itemize}
    \item verificación de que la serie DWI tenga volúmenes $b=0$ y direcciones de difusión válidas;
    \item revisión de archivos \texttt{bval}/\texttt{bvec};
    \item confirmación de que \texttt{FA\_en\_T1.nii.gz}, \texttt{ColorFA\_en\_T1.nii.gz}, mapas de densidad y \texttt{wholebrain\_T1.trk} estén alineados con \texttt{rT1.nii};
    \item validación visual de M1, M2, tronco encefálico y waypoints por hemisferio;
    \item revisión de número de streamlines retenidas por hemisferio;
    \item advertencia explícita cuando solo existe una vía parcial hasta puente o mesencéfalo;
    \item registro en JSON de rutas, parámetros y advertencias.
\end{itemize}
...
% ============================================================
\section{Procesamiento de EMG}

\subsection{Señal cruda}
...
\subsection{Frecuencia dominante}
...
% ============================================================
\section{Procesamiento de dinamometría}

Sea \(F(t)\) la curva de fuerza.

\subsection{Fuerza máxima}

\[
F_{\max}=\max_t F(t).
\]

\subsection{Fuerza media}
...
\subsection{Asimetría de fuerza}
...
% ============================================================
\section{Consolidación de métricas}
...
\begin{itemize}
    ...
    \item \(t\): etapa.
    \item \(q\): modalidad.
    \item \(m\): nombre de métrica.
    \item \(v\): valor.
    \item \(u\): unidad o escala.
\end{itemize}
...
% ============================================================
\section{Normalización estadística}
...
\begin{itemize}
    \item \(z=0\): valor promedio.
    \item \(z>0\): por encima del promedio.
    \item \(z<0\): por debajo del promedio.
\end{itemize}
...
\subsection{Índice de recuperación hacia sano}
...
% ============================================================
\section{Modelos matemáticos ampliados y explicación de ecuaciones}
...
% ------------------------------------------------------------
\subsection{Notación unificada del pipeline}
...
% ------------------------------------------------------------
\subsection{Modelo físico de intensidad en TAC}
...
\begin{itemize}
    \item \(\mu\) es el coeficiente de atenuación del tejido.
    \item \(\mu_{\text{agua}}\) es la atenuación del agua.
    \item \(\mu_{\text{aire}}\) es la atenuación del aire.
\end{itemize}
...
\begin{itemize}
    \item \(PV(\mathbf{x})\) es el valor crudo almacenado en el DICOM.
    \item \(m\) es \texttt{RescaleSlope}.
    \item \(b\) es \texttt{RescaleIntercept}.
\end{itemize}
...
% ------------------------------------------------------------
\subsection{Modelo de umbralización tisular}
...
% ------------------------------------------------------------
\subsection{Modelo geométrico voxel--mundo físico}
...
% ------------------------------------------------------------
\subsection{Modelo de remuestreo}
...
% ------------------------------------------------------------
\subsection{Modelo morfológico para limpieza de máscaras}
...
% ------------------------------------------------------------
\subsection{Modelo de componentes conectados}
...
% ------------------------------------------------------------
\subsection{Modelo anatómico de landmarks}
...
% ------------------------------------------------------------
\subsection{Modelo de ray casting muscular}
...
% ------------------------------------------------------------
\subsection{Modelo wavelet CWT para detección de borde}
...
% ------------------------------------------------------------
\subsection{Modelo de área, perímetro y espesor subcutáneo}
...
% ------------------------------------------------------------
\subsection{Modelo de volumen y propagación de error}
...
% ------------------------------------------------------------
\subsection{Modelo de corrección cortical por optimización}
...
% ------------------------------------------------------------
\subsection{Métricas de solapamiento espacial}
...
% ------------------------------------------------------------
\subsection{Modelo de atlas determinístico}
...
% ------------------------------------------------------------
\subsection{Modelo de atlas probabilístico}
...
% ------------------------------------------------------------
\subsection{Modelo MNI--paciente}
...
\begin{itemize}
    \item \(A_f\) es el área de una cara o elemento de superficie.
    \item \(\bar{T}_f\) es el grosor medio asociado a esa cara.
\end{itemize}
...
% ------------------------------------------------------------
\subsection{Modelo de intersección anatómico--funcional}
...
% ------------------------------------------------------------
\subsection{Modelo GLM para fMRI}
...
\begin{itemize}
    \item \(\mathbf{y}\) es la serie temporal BOLD de un voxel.
    \item \(X\) es la matriz de diseño experimental.
    \item \(\boldsymbol{\beta}\) son los coeficientes del modelo.
    \item \(\boldsymbol{\varepsilon}\) es el error.
\end{itemize}
...
% ------------------------------------------------------------
\subsection{Modelo de filtrado EMG}
...
% ------------------------------------------------------------
\subsection{Modelo temporal--frecuencial EMG}
...
% ------------------------------------------------------------
\subsection{Modelo de sincronización neuromuscular}
...
% ------------------------------------------------------------
\subsection{Modelo dinámico de fuerza}
...
% ------------------------------------------------------------
\subsection{Modelo estadístico longitudinal}
...
% ------------------------------------------------------------
\subsection{Modelo de distancia al sano}
...
% ------------------------------------------------------------
\subsection{Modelo de confiabilidad y estabilidad}
...
\begin{itemize}
    \item \(\sigma_s^2\) es la varianza entre sujetos.
    \item \(\sigma_e^2\) es la varianza de error.
\end{itemize}

Un \(ICC\) cercano a 1 indica alta confiabilidad.

% ------------------------------------------------------------
\subsection{Modelo de decisión integrada}
...
\begin{itemize}
    \item Máscaras NIfTI de músculo, grasa y corteza.
    \item Overlays anatómicos.
    \item PNG de previsualización.
    \item CSV de métricas.
    \item Reportes JSON de corrección cortical.
    \item Archivos \texttt{.trk}, mapas de densidad, mapas RGB y labelmaps coloreados de CST.
    \item Waypoints de mesencéfalo, puente y bulbo para validación de tractografía.
    \item Manifiestos de archivos generados.
\end{itemize}
...
% ============================================================
\section{Flujo completo del pipeline}
...
\begin{itemize}
    ...
    \item Los rangos HU son aproximaciones y dependen de calidad de adquisición.
    \item El atlas-prior heurístico no reemplaza FreeSurfer ni un registro anatómico completo.
    \item MNI es útil para estandarización, pero la medición final de volumen debe preferirse en espacio nativo.
    \item La señal BOLD no mide volumen directamente; debe integrarse con anatomía estructural.
    \item La tractografía estima trayectorias probables a partir de difusión; no representa axones individuales y depende de calidad DWI, gradientes, registro, modelo local y criterios de filtrado.
    \item Con pocos pacientes, los tamaños de efecto y la dirección del cambio pueden ser más informativos que el p-value.
\end{itemize}
